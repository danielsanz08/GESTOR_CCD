                ...
                if productos.exists():
                    ...
                    for producto in productos:
                        for dev in producto.devoluciones.all():
                            devoluciones_info.append(f"{dev.cantidad_devuelta} {producto.producto.nombre}")
                    devoluciones_raw = wrap_text_p(",".join(devoluciones_info)) if devoluciones_info else wrap_text_p("Sin devoluciones")
                else:
...
def reporte_pedidos_excel_caf(request):
    ...
    columnas_anchos = {
        ...
        'E': 50,
        'F': 30,
        'G': 30  # Nueva columna para devoluciones
    }
    ...
    ws['A2'] = f"Listado de Pedidos - {fecha_actual}" 
    ws['A2'].font = Font(size=18)
    ws['A2'].alignment = Alignment(horizontal='center', vertical='center')
    ...
    headers = ['ID', 'Fecha', 'Estado', 'Registrado Por', 'Productos', 'Áreas', 'Devoluciones']
    ws.append(headers)
