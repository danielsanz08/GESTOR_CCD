    ...
    if not pedidos.exists():
        ...
        for pedido in pedidos:
            ...
            areas_raw = ", ".join(set([
                ...
                if pa.area and pa.area != 'No establecido'
            ])) or 'Sin área'

            devoluciones_list = []
            for producto_pedido in pedido.productos.all():
                for devolucion in producto_pedido.devoluciones.all():
                    nombre_producto = devolucion.pedido_producto.producto.nombre
                    presentacion = devolucion.pedido_producto.producto.presentacion
                    devoluciones_list.append(
                        f"{devolucion.cantidad_devuelta} {presentacion} de {nombre_producto} "
                        if presentacion else f"{devolucion.cantidad_devuelta} {nombre_producto} "
                    )
            devoluciones_raw = ", ".join(devoluciones_list) if devoluciones_list else "Sin devoluciones"
